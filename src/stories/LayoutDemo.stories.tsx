...
const UpsellLayoutStory: React.FC<UpsellLayoutStoryProps> = ({
  ...
  const stayNights = useMemo(() => {
    const { checkInDate, checkOutDate } = mockReservationInfo
...
const BookingViewPageWithOffersStory: React.FC<BrandThemeArgs> = ({
  brandPrimary = brandDefaults.primary,
  brandPrimaryForeground = brandDefaults.primaryForeground,
} = {}) => {
  const themeOverrides = useMemo(
  ...
  const labels = useMemo(() => getDefaultLabels(), [])
  const { formatPrice, calculateTotal, getUnitLabel } = useOfferPricing('€', mockReservationInfo)
  ...
  const cardData = useMemo(
    ...
    [bookedOffers, calculateTotal, formatPrice, getUnitLabel, labels, selections]
  )
  ...
  return (
    <div className="relative min-h-screen bg-muted pb-4" style={themeOverrides}>
      <div className="bg-muted max-w-6xl flex-col gap-6 px-4 py-10 mx-auto">
        <BookingBanner
          className="px-0"
          welcomeText={{ salutation: 'Welcome to Hotelverse!', greeting: 'Enjoy your stay!' }}
      ...
      <main className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-10">
        ...
        <section className="mt-2 mb-3 space-y-2 rounded-lg border border-gray-200 bg-gray-50 p-3">
          ...
          <div className="space-y-2">
            <header className="space-y-1">
              ...
              <p className="text-muted-foreground text-sm">Enhance your stay with exclusive packages and services.</p>
            </header>
...
export const BookingViewPageWithSpecialOffers: Story = {
  args: {},
  render: (args) => <BookingViewPageWithOffersStory {...args} />,
}
