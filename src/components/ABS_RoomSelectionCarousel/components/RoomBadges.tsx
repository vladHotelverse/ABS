...
const RoomBadges: React.FC<RoomBadgesProps> = ({
  ...
  const discountBadge = useMemo(() => {
    ...
    return (
      <div className="absolute top-3 right-3 bg-red-600 dark:bg-red-500 text-white py-1 px-2 rounded text-xs font-bold z-10">
        <span>{badgeText}</span>
      </div>
    )
  ...
  return (
    <>
      ...
      {isBidActive && !isSelected && (
        <div className="absolute bottom-3 left-2 z-10 bg-blue-600 dark:bg-blue-500 text-white text-xs flex items-center gap-1 py-1 px-2 rounded">
      ...
      {/* Selected Badge */}
      {isSelected && (
        <div className="absolute bottom-3 left-2 z-10 bg-green-600 dark:bg-green-500 text-white text-xs flex items-center gap-1 py-1 px-2 rounded">
